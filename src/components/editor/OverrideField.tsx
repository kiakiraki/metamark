...
export type OverrideFieldKey = 'lens' | 'location';

export interface OverrideFieldProps {
  /** Which EXIF field this instance overrides. Drives the store selectors below. */
  fieldKey: OverrideFieldKey;
  /** Visible heading text (e.g. "Lens Override", "Location"). */
  label: string;
  /** id for the heading, wired to the input via aria-labelledby. */
  labelId: string;
  /** Placeholder shown when there is no EXIF value to fall back to. */
  emptyPlaceholder: string;
  /** Help text shown under the controls. */
  description: string;
}

/**
 * Shared implementation behind LensOverrideField and LocationOverrideField.
 * The two fields differ only in which store slice they read/write and their
 * copy — everything else (debounced sync, state badge, Reset/Hide buttons)
...
export function OverrideField({
  ...
  const baseValue = useExifStore((state) =>
    imageId ? (state.normalizedData[imageId]?.[fieldKey] ?? null) : null
  );
  const override = useExifStore((state) =>
    imageId
      ? fieldKey === 'lens'
        ? state.lensOverrides[imageId]
        : state.locationOverrides[imageId]
      : undefined
  );
  const setOverride = useExifStore((state) =>
    fieldKey === 'lens' ? state.setLensOverride : state.setLocationOverride
  );
  const clearOverride = useExifStore((state) =>
    fieldKey === 'lens' ? state.clearLensOverride : state.clearLocationOverride
  );
    ...
    const handle = window.setTimeout(() => {
      ...
      if (!trimmed) {
        if (override !== undefined && override !== null) {
          clearOverride(imageId);
        }
        return;
      }
      if (override !== inputValue) {
        setOverride(imageId, inputValue);
      }
    }, DEBOUNCE_MS);
    return () => window.clearTimeout(handle);
  }, [inputValue, imageId, override, setOverride, clearOverride]);
  ...
  return (
    <div className="space-y-3">
      ...
      <input
        ...
        disabled={disabled}
        placeholder={baseValue ?? emptyPlaceholder}
        className={clsx(
